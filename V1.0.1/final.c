...
int main(){
    ...
    int programEnd = 0;
    int headWind, xWind =0 , tempC=0, altSetting=0, rwyHDG=0, rwyLEN=0, airportElevation=0, pressureAltitude=0, groundRoll=0, clear50ft=0, appSpeed=0, again = 1;
    char icao[5], icaoPath[20], rwyID[5], xWindDir[] = "N";
            ...
            //While loop to keep program running until the user ends it
                while (again == 1){
                    ...
                    while(windDir<1||windDir>361){ //Input validation for weight between 1->360
                        ...
                        if(windDir<1||windDir>361){
                            printf("Invalid Wind Direction! Min: 001DEG MAX: 360DEG\n");//comunicates to user if input is erroneous.
                        }
                    }
                    ...
                    pressureAltitude = calculatePressureAltitude(altSetting, airportElevation);//Calculates Pressure Altitude
                    headWind = headwindComponent(rwyHDG, windDir, windSpeed);//Calculates Headwind/Tailwind Component
                    xWind = crosswindComponent(rwyHDG, windDir, windSpeed, xWindDir);//Calculates Crosswind Componet
