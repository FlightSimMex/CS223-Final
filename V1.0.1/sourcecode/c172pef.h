...
void printOutput(char* ptricao, char* ptrrwyID, int appSpeed, int groundRoll, int clear50ft, int windDir, int windSpeed, int tempC, int pressureAltitude, int xWindComponent, char xWindDirection[2]){
    //Creating and opening file
    FILE *fout;
    fout = fopen("files/Output/out.txt", "w");

    fprintf(fout, "\nLanding Performance\n\t%s Runway:%s", ptricao, ptrrwyID);//Prints ICAO code and Selected Runway ID
    fprintf(fout, "\n\nOutputs:\n");
    fprintf(fout, "\tVref: %dkts\t\t\tVapp: %dkts\n\tGround Roll: %dft\t\tDistance to Clear a 50ft obstacle: %dft\nCross Wind Component:%s%dKTS", appSpeed, appSpeed + 5, groundRoll, clear50ft,xWindDirection,xWindComponent);//Prints Reference Speed, Approach Speed, Ground Roll Distance, Distance to clear a 50ft obstacle.
    fprintf(fout, "\n\nComputed Using:\n");
...
int crosswindComponent(int rwyHDG, int windDir, int windSpeed, char xwDir[2]){
